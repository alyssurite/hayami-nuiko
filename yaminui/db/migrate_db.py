...
def migrate_db() -> None:
    ...
    users = orjson.loads((src / "users.json").read_bytes())
    channels = orjson.loads((src / "channels.json").read_bytes())
    log.info("Done!")
    ...
    with Session(ENGINE) as s:
        ...
        for user in users:
            s.get(User, user["id"]).forward_mode = forward_mode
        s.commit()
    log.info("Done!")
    ...
    artposts = orjson.loads((src / "artworks.json").read_bytes())
    with Session(ENGINE) as s:
...
def old_migrate_db() -> None:
    ...
    for path in dirs:
        ...
        messages = orjson.loads((path / "result.json").read_bytes())["messages"]
        with Session(ENGINE) as s:
